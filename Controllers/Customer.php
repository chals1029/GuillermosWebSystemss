...
if (isset($_GET['action']) && in_array($_GET['action'], ['check_feedback'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (isset($_POST['reorder'])) {
        ...
        // Fixed query: Use orders table, correct columns/table, join to get items
        $sql = "SELECT p.Product_Name AS name, od.unitprice AS price, od.Quantity AS quantity
                FROM order_detail od
                JOIN orders o ON od.Order_ID = o.OrderID
                JOIN product p ON od.Product_ID = p.Product_ID
                WHERE o.OrderID = ? AND o.User_ID = ? AND o.Status = 'Completed'";
        $stmt = $conn->prepare($sql);
        ...
        if ($items) {
            ...
            foreach ($items as $item) {
                ...
                // Get product
                $sql = "SELECT Product_Name, Price, Image FROM product WHERE Product_Name = ? LIMIT 1";
                $stmt = $conn->prepare($sql);
    ...
    if (isset($_POST['checkout'])) {
        ...
        // Insert invoice. Handle schemas where Invoice_ID is NOT NULL and not AUTO_INCREMENT.
        $invoiceId = 1;
        $invoiceIdResult = $conn->query("SELECT COALESCE(MAX(Invoice_ID), 0) + 1 AS NextInvoiceID FROM invoice");
        if ($invoiceIdResult) {
            $invoiceIdRow = $invoiceIdResult->fetch_assoc();
            $invoiceId = (int)($invoiceIdRow['NextInvoiceID'] ?? 1);
        }

        $sql = "INSERT INTO invoice (Invoice_ID, User_ID, Invoice_Date, Total, Customer_Change, Invoice_Status, Mode_Payment) VALUES (?, ?, NOW(), ?, ?, 'Completed', ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('iiids', $invoiceId, $userId, $total, $change, $method);
        $stmt->execute();
...
function renderPurchaseHistory(orders) {
    ...
    orders.forEach(order => {
        ...
        dateTime.innerHTML = `${date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}<small>${date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</small>`;
        ...
        if (order.Status === 'Completed') statusBadge.classList.add('status-completed');
        else if (order.Status === 'Pending') statusBadge.classList.add('status-pending');
        ...
        if (order.Status === 'Completed') {
            ...
            btn.onclick = () => cancelOrder(order.OrderID, order.Order_Date);
        ...
        html += `
            <div class="cart-item" style="--item-index: ${index++};">
                ...
                <div class="quantity-controls">
                    ...
                    <button class="quantity-btn" data-action="increase" data-product="${name}">+</button>
                </div>
...
function addToCart(productId, name, price, image) {
    ...
    fetch(location.href, { method: 'POST', body: fd })
        .then(() => {
            if (!cart[name]) {
                cart[name] = { product_id: productId || null, price: price, quantity: 0, image: image };
            } else {
...
function openCreateReservation() {
    document.getElementById('create-reservation-modal').style.display = 'flex';
    // Set minimum date to today
    const now = new Date();
    now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
    document.getElementById('reservation-date-input').min = now.toISOString().slice(0, 16);
}
...
function loadReservations() {
    ...
    .then(res => {
        if (res.status === 'success') {
            ...
            alert('Error loading reservations: ' + res.message);
        }
    })
    .catch(err => {
        console.error(err);
        alert('Network error loading reservations.');
    });
}
...
function renderReservations(reservations) {
    ...
    reservations.forEach(res => {
        ...
        idDiv.className = 'reservation-id';
        idDiv.innerHTML = `Reservation #${res.Reservation_ID}`;
        
        const dateDiv = document.createElement('div');
        dateDiv.className = 'reservation-date';
        dateDiv.textContent = date.toLocaleString('en-PH', { 
        ...
        const statusBadge = document.createElement('span');
        statusBadge.className = 'status-badge-res';
        if (res.Payment_Status === 'Confirmed') statusBadge.classList.add('status-confirmed');
        else if (res.Payment_Status === 'Pending') statusBadge.classList.add('status-pending');
        else if (res.Payment_Status === 'Cancelled') statusBadge.classList.add('status-cancelled');
        else if (res.Payment_Status === 'Complete') statusBadge.classList.add('status-completed');
        statusBadge.textContent = res.Payment_Status;
        ...
        dateItem.innerHTML = `
            ...
            <div class="info-value">${date.toLocaleDateString('en-PH', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
        `;
        ...
        timeItem.innerHTML = `
            ...
            <div class="info-value">${date.toLocaleTimeString('en-PH', { hour: 'numeric', minute: '2-digit', hour12: true })}</div>
        `;
        ...
        statusItem.innerHTML = `
            ...
            <div class="info-value">${res.Payment_Status}</div>
        `;
        ...
        if (res.Payment_Status === 'Pending') {
            ...
            cancelBtn.className = 'action-btn cancel-order';
            cancelBtn.textContent = 'Cancel Reservation';
            cancelBtn.onclick = () => cancelReservation(res.Reservation_ID);
            ...
            if (res.status === 'success') {
                ...
                $('delivery-message').innerHTML = `
                    ...
                    <div style="background:#f8f5f0;padding:20px;border-radius:12px;border-left:4px solid #6B4F3F;margin:20px 0;">
                        ...
                        <div style="margin-bottom:10px;"><strong>Status:</strong> <span style="color:#28a745;font-weight:600;">Confirmed & Being Prepared</span></div>
                    </div>
        ...
        card.innerHTML = `
            ...
            <div class="reservation-body">
                ...
                <div class="reservation-actions">
                    <button class="btn btn-secondary" onclick="cancelReservation(${reservation.Reservation_ID})" style="padding:8px 16px;border-radius:6px;border:1px solid #dc3545;color:#dc3545;background:#fff;">
...
async function checkFeedbackStatus(orderId) {
    try {
        const response = await fetch(`?action=check_feedback&order_id=${orderId}`);
